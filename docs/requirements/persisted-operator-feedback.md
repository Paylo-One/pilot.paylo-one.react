...
The request can be lost, delayed, or return after a commit. The client performs no automatic retry. A caller-generated UUID is the idempotency key; duplicate inserts are reconciled by reading the authoritative row and accepting only an exact tenant/user/payload match. Ordering is irrelevant because events are append-only observations. There is one database write and no dual write. Old code and existing rows remain compatible; rollback is removal of the new action invocation, with retained events harmless and readable by the unchanged schema. Rollout abort signal: feedback write errors or tenant-isolation failures; rollback by reverting the application commit.

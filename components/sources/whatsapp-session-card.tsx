 ...
 *
 * Scaffold boundary (ADR-036, validation-gated): there is **no real WhatsApp
 * session, QR, credentials, or message ingestion**. "Simulate scan" flips the
 * persisted status so the connected UX is explorable; chat discovery uses mock
 * chats (no bridge yet), but the operator's monitor selections are real.
 *
  ...
  return (
    <div className="wa-session">
      ...
      <p className="scaffold-note" style={{ marginBottom: "var(--space-md)" }}>
        ...
        chats. Scaffold — session state + selections persist, but there is no real
        session, credentials, or messages yet (Web-session bridge pending
        validation, ADR-036).
      </p>

      {onboarding ? (
        <>
          <WhatsAppQrOnboarding onCancel={() => run(disconnectWhatsAppAction)} />
        ...
        <>
          <div className="integration__meta">
            <div className="meta-row">
              ...
              <span className="meta-row__value">healthy · scaffold</span>
            </div>
            <div className="meta-row">
          ...
          <div className="integration__detail-block">
            ...
            <p className="segmented__hint">
              Chat discovery is mock until the bridge is validated — your selections persist.
            </p>
          </div>
        </>
      ) : null}
